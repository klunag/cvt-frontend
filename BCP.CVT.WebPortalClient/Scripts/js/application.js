...
(function ($) {
  ...
  var App = {
    ...
    handleWindowScroll: function handleWindowScroll(evt) {
      var currPos = window.pageYOffset;
      var alpha1 = Math.min(0.090, currPos * 0.001);
      var alpha2 = Math.min(0.075, currPos * 0.001);
      this.$header.css({
        "box-shadow": "inset 0 1px 0 rgba(255,255,255, " + alpha1 + ")," + "0 1px 5px rgba(0, 0, 0, " + alpha2 + ")"
      });
    },
    ...
    collapseSidenav: function collapseSidenav() {
      ...
      this.$sidenavBtn.attr('aria-expanded', false).attr('title', 'Expandir menu');
    },
