...
function listarTecNoReg() {
    ...
    $table.bootstrapTable({
        locale: 'es-SP',
        url: URL_API_VISTA + "/ListadoEquiposNoRegistrados",
        method: 'POST',
        ...
        queryParams: function (p) {
            ...
            DATA_EXPORTAR.motivo = $.trim($("#cbFilEq").val());
            DATA_EXPORTAR.origen = $("#cbFilTipEq").val();
            DATA_EXPORTAR.flagAprobado = $.trim($("#cbFilFlagAprobado").val()) == '' ? null : $.trim($("#cbFilFlagAprobado").val());
...
function ExportarRegistros() {
    ...
    let url = String.Format("{0}/ExportarEquipos?nombre={1}&motivo={2}&origen={3}&flagAprobado={4}&sortName={5}&sortOrder={6}",
        URL_API_VISTA,
        DATA_EXPORTAR.nombre,
        DATA_EXPORTAR.motivo,
        DATA_EXPORTAR.origen,
        DATA_EXPORTAR.flagAprobado || '',
...
function opcionesFormatter(data, row) {
    let opciones = '';

    if (row.Origen != origenQualys) return opciones;
    if (row.FlagAprobado != null) return opciones;

    let nombreEquipo = (row.NombreEquipo || '') == '' ? row.IPEquipo : row.NombreEquipo;

    opciones = `<div class="opciones-formatter">
                    <a href="#" class="btn btn-xs btn-success" title="Aprobar" onclick="actualizarFlagAprobado(${row.EquipoNoRegistradoId}, '${nombreEquipo}', true)">✔</a>
                    &nbsp;
                    <a href="#" class="btn btn-xs btn-danger" title="Rechazar" onclick="actualizarFlagAprobado(${row.EquipoNoRegistradoId}, '${nombreEquipo}', false)">✖</a>
                </div>`;

    return opciones;
}

function actualizarFlagAprobado(equipoNoRegistradoId, equipoStr, flagAprobado) {
    ...
    bootbox.confirm({
        ...
        callback: (result) => {
            ...
            let url = `${URL_API_VISTA}/GetUpdateEquipoNoRegistradoFlagAprobado?equipoNoRegistradoId=${equipoNoRegistradoId}&equipoStr=${encodeURIComponent(equipoStr)}&flagAprobado=${flagAprobado}`;
            ...
            fetch(url, init)
                ...
                .then(data => {
                    waitingDialog.hide();
                    if (data == true) {
                        bootbox.alert(`Se ${(flagAprobado == true ? "aprobó" : "rechazó")} correctamente.`, buscarTecNoReg);
                        return;
                    }
