...
var $tableFlujosAprobacion2 = $("#tblFlujosAprobacion2");
...
var ULTIMO_SORT_NAME2 = "applicationId";
...
$(function () {
    ...
    $("#btnExportar").click(ExportarInfo2);
    //ValidarCampos();
    //validarFormImportar();
    CargarCombos();
...
function ListarRegistros() {
    ...
    nombre_app = $("#hdAplicacionId").val() !== "0" ? $("#hdAplicacionId").val() : $("#txtAplicacion").val();
    waitingDialog.show("Procesando...", { dialogSize: "sm", progressType: "warning" });
    $table.bootstrapTable('destroy');
    $table.bootstrapTable({
        url: URL_API_GESTION + "/ListarCatalogoAplicacion",
        ...
        queryParams: function (p) {
            ...
            DATA_EXPORTAR.AppReactivadas = CaseIsNullSendExport($("#ddlAppReactivadas").val());
...
function opcionesFormatter(value, row) {
    ...
    verFicha = `<a href="FichaAplicacion?id=${row.AppId}&&nom_App=${nombre_app}&paginaActual=${ULTIMO_PAGE_NUMBER}&paginaTamanio=${ULTIMO_REGISTRO_PAGINACION}" title="Ver ficha y editar"><i class="glyphicon glyphicon-share table-icon"></i></a>`;
    ...
    if (row.isReactivated == true) {
        appReactivada = `<a href="#" title="${row.EstadoReactivacion}"><i class="iconoRojo glyphicon glyphicon-asterisk"></i></a>`;
        return verFicha.concat("&nbsp;&nbsp;", verFlujos).concat("&nbsp;&nbsp;", appReactivada);
    }
    else if (row.SolPendiente == 1) {
        if (row.TipoSol == 3) { conSolicitud = `<a href="#" title="Aplicación con una solicitud de eliminación pendiente"><i class="iconoRojo glyphicon glyphicon-envelope"></i></a>`; }
        else if (row.TipoSol == 4) { conSolicitud = `<a href="#" title="Aplicación con una solicitud de modificación pendiente"><i class="iconoRojo glyphicon glyphicon-envelope"></i></a>`; }
        else if (row.TipoSol == 5) { conSolicitud = `<a href="#" title="Aplicación con una solicitud de reversión de eliminación pendiente"><i class="iconoRojo glyphicon glyphicon-envelope"></i></a>`; }
        return verFicha.concat("&nbsp;&nbsp;", verFlujos).concat("&nbsp;&nbsp;", conSolicitud);
    }
    else
...
function DescargarAppsActualizar() {
    let url = `${URL_API_VISTA}/GestionAplicacion/Exportar/Actualizar`;
    $.ajax({
        url: url,
...
function getColumnasBT() {
    ...
    $.ajax({
        type: "GET",
        contentType: "application/json; charset=utf-8",
        url: URL_API_GESTION + `/GetColumnaAplicacionToJSCatalogo?tablaProcedencia=${tablaProcedencia}`,
        beforeSend: function (xhr) { xhr.setRequestHeader('Authorization', 'Bearer ' + localStorage.getItem('token')); },
        dataType: "json",
        success: function (dataObject, textStatus) {
            if (textStatus === "success") {
...
function ExportarInfo2() {
    ...
    let url = `${URL_API_VISTA}/GestionAplicacion/Exportar2?gerencia=${DATA_EXPORTAR.Gerencia}&division=${DATA_EXPORTAR.Division}&unidad=${DATA_EXPORTAR.Unidad}&area=${DATA_EXPORTAR.Area}&estado=${DATA_EXPORTAR.Estado}&clasificacionTecnica=${DATA_EXPORTAR.ClasificacionTecnica}&subclasificacionTecnica=${DATA_EXPORTAR.SubclasificacionTecnica}&aplicacion=${DATA_EXPORTAR.Aplicacion}&sortName=${DATA_EXPORTAR.sortName}&sortOrder=${DATA_EXPORTAR.sortOrder}&TablaProcedencia=${DATA_EXPORTAR.TablaProcedencia}&Procedencia=${DATA_EXPORTAR.Procedencia}&TipoActivo=${DATA_EXPORTAR.TipoActivo}&TipoPCI=${DATA_EXPORTAR.TipoPCI}&Exportar=1&AppReactivada=${DATA_EXPORTAR.AppReactivadas}&GestionadoPor=${DATA_EXPORTAR.GestionadoPor}&LiderUsuario=${DATA_EXPORTAR.LiderUsuario}`;
    $.ajax({
        url: url,
        contentType: "application/vnd.ms-excel",
        beforeSend: function (xhr) {
...
function verFlujosAplicacion(id) {
    ...
    ListarFlujosAprobacionPendientes(id);
...
function ListarFlujosAprobacionPendientes(id) {
    $tableFlujosAprobacion2.bootstrapTable('destroy');
    $tableFlujosAprobacion2.bootstrapTable({
        locale: 'es-SP',
        url: URL_API_VISTA + `/application/flowsPendiente`,
        ajaxOptions: { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } },
        method: 'POST',
        pagination: true,
        sidePagination: 'server',
        ...
        pageSize: 10,
        pageList: OPCIONES_PAGINACION,
        sortName: "NombreUsuarioCreacion",
        sortOrder: "asc",
        queryParams: function (p) {
