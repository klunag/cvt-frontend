...
const MENSAJE_RECHAZAR_SOLICITANTE2 = "¿Desea continuar con el envío de la observación?";
...
function ListarRegistros() {
    ...
    $table.bootstrapTable({
        locale: 'es-SP',
        url: URL_API_VISTA + `/application/requests/creation`,
        ...
        queryParams: function (p) {
            ...
            DATA_EXPORTAR.registrationSituation = $("#cbFilSituacionRegistro").val();
...
function VerAplicacion(id) {
    ...
    LimpiarValidateErrores($("#formRechazarSolicitante2"));
    editarAplicacion(id);
}

function editarAplicacion(Id) {
    document.getElementById("txtDescripcionRechazoSolicitante2").value = "";
    document.getElementById("txtCodigoAppPadre").value = "";
    document.getElementById("hCodigoPadre").value = "";
    document.getElementById("txtCodigoInterfaz").value = "";
    document.getElementById("txtRegistroUmbral").value = "";
    $("#ckbInterface").removeAttr("disabled");
    $("#ckbInterface").prop("checked", false);
    $("#ckbProveedorDesarrollo").removeAttr("disabled");
    $("#ckbProveedorDesarrollo").prop("checked", false);
    $("#ckbFlagPiezaCross").removeAttr("disabled");
    $("#ckbFlagPiezaCross").prop("checked", false);
        ...
        success: function (dataObject, textStatus) {
            if (textStatus === "success") {
                if (dataObject !== null) {
                    ...
                    $("#ddlArquitectoSolucion").val(data.architectSolutionId || "0");
                    ...
                    $("#txtDescripcionRechazoSolicitante2").val(data.commentsObserved);
...
function opcionesFormatter(value, row, index) {
    if (row.registrationSituation === REGISTRO_COMPLETO) {
        let btnConfirmar = `<a href="javascript:irConfirmar(${row.id})" title="Confirma el registro de la aplicación"><i class="iconoVerde glyphicon glyphicon glyphicon-ok"></i></a>`;
        let btnRechazarSolicitante = `<a href="javascript:irRechazoSolicitante(${row.id})" title="Observar el registro del solicitante"><i class="iconoRojo glyphicon glyphicon glyphicon-remove"></i></a>`;
        let btnObservar = `<a href="javascript:irObservar(${row.id})" title="Revisa el flujo de aprobación y valida los datos de cada custodio"><i class="iconoRojo glyphicon glyphicon glyphicon-retweet"></i></a>`;

        return btnConfirmar.concat("&nbsp;&nbsp;", btnRechazarSolicitante.concat("&nbsp;&nbsp;", btnObservar));
    }
    else {
        let btnRechazarSolicitante = `<a href="javascript:irRechazoSolicitante(${row.id})" title="Observar el registro del solicitante"><i class="iconoRojo glyphicon glyphicon glyphicon-remove"></i></a>`;
        let btnObservar = `<a href="javascript:irObservar(${row.id})" title="Revisa el flujo de aprobación y valida los datos de cada custodio"><i class="iconoRojo glyphicon glyphicon glyphicon-retweet"></i></a>`;
        let btnEliminar = `<a href="javascript:irDesactivar(${row.id})" title="Desactivar la aplicación de manera permanente""><i class="iconoRojo glyphicon glyphicon-trash table-icon"></i></a>`;
        return btnEliminar.concat("&nbsp;&nbsp;", btnRechazarSolicitante.concat("&nbsp;&nbsp;", btnObservar));
    }
}

function opcionesFlujoFormatter(value, row, index) {
    let btnObservar = `<a href="javascript:irRechazar(${row.FlowAppId})" title="Observa este flujo de registro y notifica al responsable"><i class="iconoRojo glyphicon glyphicon glyphicon-remove"></i></a>`;
    return btnObservar;
}
...
function guardarRechazoAppSolicitante2() {
    if ($("#formRechazarSolicitante2").valid()) {
        bootbox.confirm({
            title: TITULO_MENSAJE,
            ...
            callback: function (result) {
                result = result || null;
                if (result !== null && result) {
                    sendDataFormAPISolicitante2($("#btnRegistrarAppSolicitante"));
                }
            }
        });
    }
...
function validarFormRechazoSolicitante2() {
    $("#formRechazarSolicitante2").validate({
        validClass: "my-valid-class",
        errorClass: "my-error-class",
        ignore: ".ignore",
        highlight: function (element) {
        ...
        rules: {
            txtDescripcionRechazoSolicitante2: {
                requiredSinEspacios: true,
                maxlength: 800
            }
        },
        ...
        errorPlacement: (error, element) => {
            if (element.attr('name') === "txtDescripcionRechazoSolicitante2") {
                // element.parent().parent().append(error);
                element.parent().append(error);
            } else {
                element.parent().append(error);
...
function sendDataFormAPISolicitante2($btn) {
    var estadoTransaccion = true;
    if ($btn !== null) {
        $btn.button("loading");
        waitingDialog.show("Procesando...", { dialogSize: "sm", progressType: "warning" });
...
function CrearObjAplicacion2() {
    var data = {};
    data.AppId = $("#hdAplicacionId").val();
    data.comments = $("#txtDescripcionRechazoSolicitante2").val();
