...
function editarAplicacion(Id) {
    ...
    $.ajax({
        url: URL_API_VISTA + `/application/Detail/${Id}`,
        type: "GET",
        beforeSend: function (xhr) { xhr.setRequestHeader('Authorization', 'Bearer ' + localStorage.getItem('token')); },
        dataType: "json",
        success: function (dataObject, textStatus) {
            if (textStatus === "success") {
                if (dataObject !== null) {
                    ...
                    $("#txtRegistradoPor").val(data.SolicitanteDetail);

                    $("#txtFecha").val(data.registerDate);
...
function ListarRoles() {
    ...
    $table.bootstrapTable({
        locale: 'es-SP',
        url: URL_API_VISTA + "/application/rolesInitial",
        method: 'POST',
