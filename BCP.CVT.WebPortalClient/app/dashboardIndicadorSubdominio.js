...
function SetValorDefaultCombos() {
    $("#cbFilTipoEquipo").val("3");

    $("#cbFilDominio").val(["9", "12"]);
    $("#cbFilDominio").multiselect("refresh");
    $("#cbFilDominio").trigger("change");

    $("#cbFilDominioRed").val(["3"]);
    $("#cbFilDominioRed").multiselect("refresh");

    $("#cbFilSubdominio").val(ARR_INDICADOR_OBS);
    $("#cbFilSubdominio").multiselect("refresh");
}
...
function ExportarData() {
    ...
    if ($("#formFiltros").valid()) {
        ...
        let url = `${URL_API_VISTA2}/Reportes/GraficoEvolucion/Exportar?fecha=${fecha}&tipoEquipoId=${tipoEquipoId}&subdominioFiltrar=${subdominioFiltrar}&subsidiaria=${subsidiariaFiltrar}`;
        $.ajax({
            url: url,
...
function listarRegistros() {
    ...
    $table.bootstrapTable({
        ...
        queryParams: function (p) {
            ...
            DATA_EXPORTAR.TipoEquipoId = $("#cbFilTipoEquipo").val();
