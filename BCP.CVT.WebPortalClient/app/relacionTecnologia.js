...
function opcionesFormatter(value, row) {
    ...
    if (row.Aprobar === 'true') {
        if (row.EstadoId === ESTADO_RELACION.APROBADO)
            eliminar = `<a href="javascript:IrEliminarRegistrov2(${value});" title="Eliminar registro"><i style="" class="glyphicon glyphicon-trash table-icon"></i></a>`;
        else
        ...
        return eliminar.concat("&nbsp;&nbsp;", cambiarEstado).concat("&nbsp;&nbsp;", reiniciarEstado).concat("&nbsp;&nbsp;", comentarios);
    }
    else
        return '';
}
...
function linkFormatter(value, row) {
    return `<a href="javascript:EditRegistro(${row.RelacionId},${row.EstadoId}, '${row.EstadoStr}')" title="Editar">${value}</a>`;
...
function IrEliminarRegistrov2(Id) {
    LimpiarValidateErrores($("#formOS"));
    $("#hdId").val(Id);
    $("#ddlMotivo").val("-1");
    $("#txtObs").val("");
...
function EditRegistro(Id, EstadoId, EstadoStr) {
    if (EstadoId === ESTADO_RELACION.PENDIENTE || EstadoId === ESTADO_RELACION.APROBADO) {
        $("#titleForm").html("Editar Relación Tecnología");
        $.ajax({
