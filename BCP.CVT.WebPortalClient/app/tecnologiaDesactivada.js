...
var DATA_TECNOLOGIA = [];
var DATA_APLICACION = [];
var FLAG_ACTIVO_TECNOLOGIA = null;
...
$(function () {
    ...
    $("#cbFilDom").on('change', function () {
        //getSubdominiosByDomCb(this.value, $("#cbFilSub"));
    ...
    InitAutocompletarTecnologiaForBusqueda($("#txtBusTec"), null, $(".searchContainer"), null, false);
...
function validarFormEquivalencias() {
    ...
    $("#formTecByArq").validate({
        ...
        messages: {
            txtNomTecEq: {
                required: String.Format("Debes ingresar {0}.", "una equivalencia"),
...
function listarTecSTD() {
    ...
    $table.bootstrapTable({
        ...
        queryParams: function (p) {
            ...
            DATA_EXPORTAR.flagActivo = null;//FLAG_ACTIVO_TECNOLOGIA;//$("#cbFilEstObs").val();
...
function cambiarEstado(TecnologiaId, ClaveTecnologia) {

    $("#txtClaveTecnologia").val(ClaveTecnologia);
    $("#hIdTec").val(TecnologiaId);
    FLAG_ACTIVO_TECNOLOGIA = 0;

    let estado = ExisteClaveTecnologia();
    if (estado) {
        FLAG_ACTIVO_TECNOLOGIA = null;
        bootbox.alert({
            size: "sm",
            title: TITULO_MENSAJE,
            message: "Ya existe una tecnología activa con la misma Clave, no es posible cambiar de estado",//String.Format("La tecnología se encuentra en estado {0}, no es posible editar.", EstadoTecnologiaStr),
            buttons: {
                ok: {
                    label: 'Aceptar',
...
function formatOpcSTD(value, row, index) {
    ...
    style_color = 'iconoRojo';
    let type_icon = row.Activo ? "check" : "unchecked";
    //let type_icon = "";
    let estado = `<a href='javascript:cambiarEstado(${row.Id}, "${row.ClaveTecnologia}")' title="Cambiar estado"><i style="" id="cbOpcTec${row.Id}" class="${style_color} glyphicon glyphicon-${type_icon}"></i></a>`;
    //let estado = '';
    ...
    //return estado.concat(estadoTec).concat(equivalencia);
    return estado;
}
...
function editarTecSTD(TecId, estadoId) {
    ...
    waitingDialog.show("Procesando...", { dialogSize: "sm", progressType: "warning" });
    ItemsRemoveAutId = [];
    FLAG_ACTIVO_TECNOLOGIA = null; //TECNOLOGIAS INACTIVAS
    //$tblAutTec.bootstrapTable('destroy');
    //$tblAutTec.bootstrapTable();
...
function exportarTecSTD() {
    ...
    //let url = `${URL_API_VISTA}/ExportarSTD?nombre=${DATA_EXPORTAR.nombre}&dominioId=${DATA_EXPORTAR.domId}&subdominioId=${DATA_EXPORTAR.subdomId}&familiaId=${DATA_EXPORTAR.famId}&estadoFecSop=${DATA_EXPORTAR.fecId}&casoUso=${DATA_EXPORTAR.casoUso}&estadoTec=${DATA_EXPORTAR.estadoId}&aplica=${DATA_EXPORTAR.aplica}&codigo=${DATA_EXPORTAR.codigo}&dueno=${DATA_EXPORTAR.dueno}&equipo=${DATA_EXPORTAR.equipo}&tipoTecId=${DATA_EXPORTAR.tipoTecId}&estObsId=${DATA_EXPORTAR.estObsId}&flagActivo=${DATA_EXPORTAR.flagActivo}&sortName=${DATA_EXPORTAR.sortName}&sortOrder=${DATA_EXPORTAR.sortOrder}`;
    ...
    $searchBox.autocomplete({
        ...
        source: function (request, response) {
            if ($.trim(request.term) !== "") {
                ...
                $.ajax({
                    //url: URL_API_VISTA + "/GetTecnologiaForBusqueda?filtro=" + request.term,
