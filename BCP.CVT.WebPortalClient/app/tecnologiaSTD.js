...
const MENSAJE_EXISTE_EQUIVALENCIA = "No es posible desactivar el registro ya que tiene equivalencias activas";
const MENSAJE_EXISTE_RELACION = "Existen relaciones activas para esta tecnología, no es posible desactivarla hasta que estás relaciones se desaprueben o eliminen";
...
$(function () {
    ...
    if (NOMBRE_TECNOLOGIA !== "") $("#txtBusTec").val(NOMBRE_TECNOLOGIA);
...
function validarMatriculaResponsable() {
    ...
    switch (idResp) {
        case IDS_RESPONSABLES_TECNOLOGIA.OWNER:
            validateResponsable($("#txtDueTec"), $("#hdMatriculaOwner"), "dueño de la tecnología");
            break;
        case IDS_RESPONSABLES_TECNOLOGIA.ARQ_SEGURIDAD:
            validateResponsable($("#txtConfArqSegTec"), $("#hdArqSeguridad"), "arquitecto de seguridad");
            break;
        case IDS_RESPONSABLES_TECNOLOGIA.ARQ_TECNOLOGIA:
            validateResponsable($("#txtConfArqTec"), $("#hdArqTecnologia"), "arquitecto de tecnología");
            break;
    }
}
...
function InitHiddenAutocomplete() {
    InitHidden($("#txtDueTec"), $("#hdMatriculaOwner"), "");
    InitHidden($("#txtConfArqSegTec"), $("#hdArqSeguridad"), "");
    InitHidden($("#txtConfArqTec"), $("#hdArqTecnologia"), "");
}

function InitAutocompleteResponsables() {
    InitAutocompletarBuilderLocal($("#txtDueTec"), $("#hdMatriculaOwner"), ".ownerContainer", "/ConfiguracionPortafolioAplicaciones/ObtenerUsuariosPorNombreAutocomplete?filter_name={0}");
    InitAutocompletarBuilderLocal($("#txtConfArqSegTec"), $("#hdArqSeguridad"), ".arqSeguridadContainer", "/ConfiguracionPortafolioAplicaciones/ObtenerUsuariosPorNombreAutocomplete?filter_name={0}");
    InitAutocompletarBuilderLocal($("#txtConfArqTec"), $("#hdArqTecnologia"), ".arqTecnologiaContainer", "/ConfiguracionPortafolioAplicaciones/ObtenerUsuariosPorNombreAutocomplete?filter_name={0}");
}
...
function validarAddOrEditFormTec() {
    ...
    $.validator.addMethod("existeFamilia2", function (value, element) {
        let estado = true;
        if ($("#hFamTecId").val() === "0") {
            if ($.trim(value) !== "" && $.trim(value).length >= 3) {
...
function limpiarMdAddOrEditTec() {
    ...
    $("#txtFamTec").val('');
    $("#cbTipTec, #cbFuenteTec, #txtApliTec").val(-1);

    $("#txtCusTec, #txtReqHSTec, #txtCompaTec").val('');
...
function cambiarEstado(TecnologiaId) {
    if (ExisteEquivalenciaTecnologia(TecnologiaId)) {
        bootbox.alert({
            size: "sm",
            title: TITULO_MENSAJE,
            message: MENSAJE_EXISTE_EQUIVALENCIA,
    ...
    if (ExisteRelacionByTecnologia(TecnologiaId)) {
        bootbox.alert({
            size: "sm",
            title: TITULO_MENSAJE,
            message: MENSAJE_EXISTE_RELACION,
            buttons: SET_BUTTONS_BOOTBOX_OK
        });
...
function editarTecSTD(TecId, estadoId, editFromId) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            //TODO
            $("#txtApliTec").val(setAplicaTecnologia(result.Aplica));
            ...
            $("#txtUrlConfluence").val(result.UrlConfluence);
...
function exportarTecSTD() {
    ...
    let url = `${URL_API_VISTA}/ExportarSTD?nombre=${DATA_EXPORTAR.nombre}&dominioIds=${DATA_EXPORTAR.domIds}&subdominioIds=${DATA_EXPORTAR.subdomIds}&familiaId=${DATA_EXPORTAR.famId}&estadoFecSop=${DATA_EXPORTAR.fecId}&casoUso=${DATA_EXPORTAR.casoUso}&estadoTecs=${DATA_EXPORTAR.estadoIds}&aplica=${DATA_EXPORTAR.aplica}&codigo=${DATA_EXPORTAR.codigo}&dueno=${DATA_EXPORTAR.dueno}&equipo=${DATA_EXPORTAR.equipo}&tipoTecIds=${DATA_EXPORTAR.tipoTecIds}&estObsIds=${DATA_EXPORTAR.estObsIds}&flagActivo=${DATA_EXPORTAR.flagActivo}&sortName=${DATA_EXPORTAR.sortName}&sortOrder=${DATA_EXPORTAR.sortOrder}`;
    $.ajax({
        url: url,
...
function ExisteRelacionByTecnologia(TecnologiaId) {
    let estado = false;
...
function migrarData() {
    if ($("#formMigrarEquivalencia").valid()) {
        ...
        let TecnologiaReceptorId = $("#hIdTec").val();
